...
어려웠던 점
redis를 외부접속이 가능하도록 만드는 것이 어려웠다.
AWS의 ElastiCache을 사용하려 했으나 보안상의 문제로 AWS 내부에서만 접근이 가능하여 EC2에 직접 설치하는 과정이 어려웠다.
완성을 하고 나면 무슨 이유인지 연결이 끊기거나 EC2에 접속이 안되는 문제가 발생했다.
